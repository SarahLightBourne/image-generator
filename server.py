...
def put_label_on_image(image: ndarray, data_to_put_on_image: DataToPutOnImage) -> None:
  cv2.putText(
    image,
...
def put_labels_on_image(image: ndarray, image_data: GenerateImageInput) -> None:
...
async def generate_image_controller(image_data: GenerateImageInput = Depends()) -> Response:
  try:
    ...
    raise HTTPException(status_code=409, detail=f"Unexpected error: {str(error)}")
  else:
